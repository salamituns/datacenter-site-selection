-- promote_ingestion_run copies grid cells with an explicit column list, so
-- the new interconnection columns have to be named there or they would be
-- staged on every run and then silently dropped at publish — the staging
-- table would be right and the published table permanently null.
--
-- Only the grid_parcels block changes; everything else is as before, and
-- the stats object now reports how many cells actually matched a facility.
