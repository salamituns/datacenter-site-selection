-- ============================================================================
-- Migration: release9_unrisked_composite_phase1
-- Description: Record the unrisked screening composite alongside the risked one.
--
--   Phase 1 of docs/unrisked-composite-spec.md. Additive and behaviour-neutral:
--   the worker begins writing composite_score_unrisked, nothing reads it yet,
--   and composite_score keeps its current meaning and its current values.
--
--   Today grid_parcels stores only the risked composite — for a parcel-tier
--   region composite_score is the screening score already multiplied by
--   evidence_coverage — and the measurement behind it is not recoverable.
--   Ohio at 0.772 and Texas at 0.664 are in the position Morrow County was in,
--   just less visibly. SPE-PRMS sanctions risking an estimate but requires the
--   unrisked figure be disclosed alongside it; JORC forbids reporting
--   categories in combined form unless the individual categories are given too.
--
--   The column is nullable on purpose. Existing rows have no unrisked value
--   and one cannot be manufactured: dividing a stored risked figure by its
--   coverage factor reconstructs a number the worker never produced, which is
--   precisely the error the Morrow County republish demonstrated. NULL here
--   means "not yet republished", and Phase 3 gates on there being none left.
-- ============================================================================

ALTER TABLE public.grid_parcels
    ADD COLUMN IF NOT EXISTS composite_score_unrisked NUMERIC(6, 2);
ALTER TABLE public.stg_grid_parcels
    ADD COLUMN IF NOT EXISTS composite_score_unrisked NUMERIC(6, 2);

COMMENT ON COLUMN public.grid_parcels.composite_score_unrisked IS
    'Screening model output before any evidence-coverage risking. The '
    'measurement; composite_score is the derived decision figure. NULL on '
    'rows published before Phase 1 of docs/unrisked-composite-spec.md.';
...
BEGIN
    ...
    INSERT INTO public.grid_parcels (
        ...
        composite_score, composite_score_unrisked, evidence_coverage, evidence_tier, cluster_zone_id, cluster_label, is_prime_zone,
    ...
    SELECT
        ...
        s.composite_score, s.composite_score_unrisked, s.evidence_coverage, s.evidence_tier, s.cluster_zone_id, s.cluster_label, s.is_prime_zone,
