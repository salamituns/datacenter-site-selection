...
# ---------------------------------------------------------------------------
# state_bbox: the containment invariant the PAD-US cache rests on
# ---------------------------------------------------------------------------
#
# fetch_padus_protected caches one clip per state against state_bbox, and
# serves a county from it only when the cached box CONTAINS the county's box.
# If containment ever fails the cache silently stops working: every county
# re-downloads the state geodatabase and overwrites the clip, which is the
# behaviour this replaced. It fails as a slow sweep, not as an error, so it
# needs a test rather than an inspection.


def test_every_county_bbox_sits_inside_its_state_bbox():
    from shapely.geometry import box

    # PJM states carrying real weight, plus the two single-county states
    # whose survey windows were set by hand.
    for state in ("DE", "OH", "VA", "KY", "PA", "WV", "NJ", "MD"):
        sb = rr.state_bbox(state)
        assert sb is not None, f"{state} has no state bbox"
        state_box = box(*sb)
        for region in rr.all_regions([state]):
            assert state_box.contains(box(*region.bbox)), (
                f"{region.region_key} is not contained by the {state} bbox; "
                "the PAD-US state cache would miss for it")


def test_hand_set_survey_windows_stay_inside_their_state_bbox():
    # SURVEY_WINDOWS replaces a derived bbox with one somebody typed, so it
    # is the one place a county box can be any shape at all.
    from shapely.geometry import box

    for key in rr.SURVEY_WINDOWS:
        region = rr.resolve(key)
        assert region is not None, f"{key} no longer resolves"
        sb = rr.state_bbox(region.state)
        assert box(*sb).contains(box(*region.bbox)), (
            f"the hand-set survey window for {key} escapes its state bbox")


def test_state_bbox_is_none_for_an_unknown_code():
    assert rr.state_bbox("ZZ") is None
    assert rr.state_bbox("") is None
