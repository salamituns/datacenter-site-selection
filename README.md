...
1. **Regional screening** — 10 km² fishnet cells scored on power, water, risk, and climate factors, clustered into Prime Development Zones (HIFLD / USGS / FEMA / NOAA live data). The composite score is **weighted by evidence coverage** (Release 9): each cell stores the share of its region's parcel gates the current diligence can decide (`evidence_coverage`, 0–1; no parcel tier = 0), and the stored composite is the screening score × that factor — so an under-evidenced site cannot out-rank a fully-diligenced one. The factor is stored per cell, so the dashboard's live slider re-weighting re-applies it instead of silently un-doing it. Current coverage: Loudoun 0.994, Franklin 0.772, Taylor 0.664, Morrow 0.0.
