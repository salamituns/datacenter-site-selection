...
1. **Regional screening** — 10 km² fishnet cells scored on power, water, risk, and climate factors, clustered into Prime Development Zones (HIFLD / USGS / FEMA / NOAA live data). Ranking is **lexicographic on evidence** (Release 9): cells compare on `evidence_tier` first and score second, so a parcel-tier site out-ranks a screening-tier one whatever the two scores are. Within the parcel tier the composite is the screening score × `evidence_coverage` — the share of that region's parcel gates the current diligence can decide (Loudoun 0.994, Franklin 0.772, Taylor 0.664) — so a partially diligenced site cannot match a fully diligenced one. Both fields are stored per cell, so the dashboard's live slider re-weighting re-applies the factor instead of silently un-doing it.
